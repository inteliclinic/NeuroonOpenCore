...
struct mock_neuroon_signals_t : public INeuroonSignals {
  ...
  void next_step() {
    ...
    std::copy(eeg_v.begin() + beginning_idx, eeg_v.begin() + m_step,
              m_eeg_signal.begin());
    m_ir_signal.resize((m_step - beginning_idx) / 5);
    std::copy(ir_v.begin() + (beginning_idx / 5), ir_v.begin() + (m_step / 5),
              m_ir_signal.begin());
  }

  virtual const std::vector<double> &eeg_signal() const { return m_eeg_signal; }
  ...
  virtual ullong last_timestamp(SignalOrigin so) const { return m_step * 8; }
