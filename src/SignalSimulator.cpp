...
EegFramesSource::EegFramesSource(SignalSource<std::int16_t> signal_source,
                                 std::size_t frame_size)
    : _frame_size(frame_size) {

  auto samples = signal_source.getValues();
  ...
  for (std::size_t i = 0; i < samples.size() / _frame_size; i++) {
    ...
    _frames.push_back(std::make_shared<EegFrame>(frame));
  }
}
...
PatFramesSource::PatFramesSource(SignalSource<std::int32_t> ir_led,
  ...
  std::vector<SourceSpec> specs = {
      ir_led.spec(),        red_led.spec(), accel_x.spec(),
      accel_y.spec(),       accel_z.spec(), temperature_1.spec(),
      temperature_2.spec(),
  };
  ...
  if (ir_led.spec().option() == SourceSpec::SourceOption::CSV) {
    ...
    v = ir_led.getValues();
    any_csv_source = true;
    if (v.size() < final_source_length) {
      final_source_length = v.size();
    }
  }
  ...
  // if there was no csv sources, then adjust source length to the longest
  // default size
  ...
  if (!any_csv_source) {
    ...
    for (auto &spc : specs) {
      if (spc.default_size() > final_source_length) {
        final_source_length = spc.default_size();
      }
    }
  }
  ...
  if (ir_led.spec().option() != SourceSpec::SourceOption::CSV) {
    auto &v = led_signals[0];
    v = ir_led.getValues(final_source_length);
  }

  if (red_led.spec().option() != SourceSpec::SourceOption::CSV) {
    auto &v = led_signals[1];
    v = red_led.getValues(final_source_length);
  }

  if (accel_x.spec().option() != SourceSpec::SourceOption::CSV) {
    auto &v = accel_axes[0];
    v = accel_x.getValues(final_source_length);
  }

  if (accel_y.spec().option() != SourceSpec::SourceOption::CSV) {
    auto &v = accel_axes[1];
    v = accel_y.getValues(final_source_length);
  }
  if (accel_z.spec().option() != SourceSpec::SourceOption::CSV) {
    auto &v = accel_axes[2];
    v = accel_z.getValues(final_source_length);
  }
  if (temperature_1.spec().option() != SourceSpec::SourceOption::CSV) {
    auto &v = temperatures[0];
    v = temperature_1.getValues(final_source_length);
  }
  if (temperature_2.spec().option() != SourceSpec::SourceOption::CSV) {
    auto &v = temperatures[1];
    v = temperature_2.getValues(final_source_length);
  }
  ...
  for (int i = 0; i < final_source_length; i++) {
    ...
    _frames.push_back(std::make_shared<PatFrame>(al));
  }
}

////////////////////////////////////////// SignalSimulator

SignalSimulator::SignalSimulator() { _set_timestamp_to_now(); }
...
bool SignalSimulator::pass_time(ullong ms_to_simulate,
                                double time_passing_modifier, bool force_wait) {
  ...
  for (const auto &triple : _pipes) {
    max_time_to_wait = std::max<ullong>(EMISSION_INTERVAL_MS(triple),
  ...
  while (ms_left > 0 || ms_to_simulate == 0) {
    ...
    for (auto &t : _pipes) {
      ...
      llong emission_time =
          EMISSION_INTERVAL_MS(t) - (_time_passed % EMISSION_INTERVAL_MS(t));
      if (emission_time == 0)
        emission_time = EMISSION_INTERVAL_MS(t);
      // printf("emmission time %lld%%%d %d\n",_time_passed,
      // ss->ms_per_frame(),emission_time);
    ...
    // Wait correct amount of time (tries to include the delay caused by
    // computation)
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> elapsed = end - start;
    llong ttw = std::max<llong>(
        0, round(time_passing_modifier * time_to_wait - elapsed.count()));
    if (ttw != 0) {
    ...
    for (auto &t : minpipes) {
      ...
      PIPE_UP(*t)->passNextFrameWithTimestamp(ts);
    }
  }
...
void SignalSimulator::add_streaming_pipe(std::unique_ptr<IFrameStreamPipe> pipe,
  ...
  _pipes.push_back(
      std::make_tuple(pipe_frame_emission_interval_ms, std::move(pipe)));
}
